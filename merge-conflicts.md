...
| `CherryPicking` | `.git/CHERRY_PICK_HEAD` 文件存在且值 ≠ `rebase-merge/stopped-sha` |
| `Reverting` | `.git/REVERT_HEAD` 文件存在 |

**CherryPick 的特殊排除逻辑**：Git 历史上 rebase 用 cherry-pick 实现，rebase 中断时 `CHERRY_PICK_HEAD` 可能残留。代码比较 `CHERRY_PICK_HEAD` 与 `rebase-merge/stopped-sha`，若前者以前者为前缀则判定为 rebase 残余、不算 cherry-pick。
...
| 字段 | 含义 | shortStatus 值 |
|------|------|----------------|
| `HasInlineMergeConflicts` | 文件内有 `<<<<<<<` 等文本标记 | `UU`（双方修改）、`AA`（双方新增） |
| `HasMergeConflicts` | 广义冲突（含文件级冲突） | 上述 + `DD`（双方删）/`AU`（我们删他们加）/`UA`（他们删我们加）/`UD`/`DU` |
...
| shortStatus | 含义 | 菜单（按顺序） |
|-------------|------|----------------|
| `DD` | 双方删除 | Delete File |
| `DU` | 我们删、他们改 | Delete File → Keep File |
| `UD` | 他们删、我们改 | Delete File → Keep File |
| `AU` | 我们删、他们加 | Keep File → Delete File |
| `UA` | 他们删、我们加 | Keep File → Delete File |

- **Keep File** → `git add <file>`（stage 当前工作树版本）
- **Delete File** → `git rm <file>`（从工作树和 index 移除）

主视图也会给这些状态渲染提示信息：`DU` 和 `UD` 还会附带 `git diff --base` 的输出，帮助用户决定是保留还是删除。
